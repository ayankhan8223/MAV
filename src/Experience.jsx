...
fakeUV=fract(fakeUV+vec2(time/20.,time/10.));
  ...
  vec4 txt=texture2D(sky,newFakeUV+0.1*cnoise(vec4(fakeUV*5.,time/10.,0.),vec4(5.)));
...
export default function Experience() {
  const { scene } = useGLTF("./head_model_of_a_child.glb");
  const obj = useRef();

  useFrame((state, delta) => {
    material.uniforms.time.value += delta * 2;
    obj.current.rotation.y += delta * 0.1;
  });
  ...
  return (
    <>
      ...
      <Center>
        <primitive object={scene} scale={2} ref={obj} />
      </Center>
